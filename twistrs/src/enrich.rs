                    ...
                    match geoip.lookup::<geoip2::City>(*ip) {
                        Ok(lookup_result) => {
                            ...
                            if lookup_result.city.is_some() {
                                geoip_string.push_str(lookup_result.city.unwrap().names
                                                        .unwrap()
                                                        .get("en")
                                                        .unwrap());                                
                            }
                            ...
                            if lookup_result.country.is_some() {
                                ...
                                geoip_string.push_str(lookup_result.country.unwrap().names
                                                        .unwrap()
                                                        .get("en")
                                                        .unwrap());                                
                            }                            
                            ...
                            if lookup_result.continent.is_some() {
                                ...
                                geoip_string.push_str(lookup_result.continent.unwrap().names
                                                        .unwrap()
                                                        .get("en")
                                                        .unwrap());
                            }
        ...
        match WhoIsLookupOptions::from_string(&self.fqdn) {
            Ok(mut lookup_options) => {
                ...
                match WHOIS.lookup(lookup_options) {
                    Ok(lookup_result) => {
                        result.who_is_lookup = Some(String::from(&lookup_result.split("\r\n")
